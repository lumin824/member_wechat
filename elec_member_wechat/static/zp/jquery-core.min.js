 ...
 (function (a2, aB) {
     ...
     bG.event = {
         ...
         (function (cd) {
             ...
             function cc(cl, co) {
                 var cn = (cl & 65535) + (co & 65535),
                     cm = (cl >> 16) + (co >> 16) + (cn >> 16);
                 return (cm << 16) | (cn & 65535)
             }
             function cj(cl, cm) {
                 return (cl << cm) | (cl >>> (32 - cm))
             }
             function b0(cq, cn, cm, cl, cp, co) {
                 return cc(cj(cc(cc(cn, cq), cc(cl, co)), cp), cm)
             }
             function bZ(cn, cm, cr, cq, cl, cp, co) {
             ...
             function b1(cv, cq) {
                 cv[cq >> 5] |= 128 << ((cq) % 32);
                 cv[(((cq + 64) >>> 9) << 4) + 14] = cq;
                 var cn, cp, co, cm, cl, cu = 1732584193,
                     ct = -271733879,
                     cs = -1732584194,
                     cr = 271733878;
                 for (cn = 0; cn < cv.length; cn += 16) {
                         cp = cu;
                         co = ct;
                         cm = cs;
                         cl = cr;
             ...
             function cf(cm) {
                 var cn, cl = "";
                 for (cn = 0; cn < cm.length * 32; cn += 8) {
                     cl += String.fromCharCode((cm[cn >> 5] >>> (cn % 32)) & 255)
                 }
                 return cl
             }
             function b6(cm) {
                 var cn, cl = [];
                 cl[(cm.length >> 2) - 1] = aB;
                 for (cn = 0; cn < cl.length; cn += 1) {
                     cl[cn] = 0
                 }
                 for (cn = 0; cn < cm.length * 8; cn += 8) {
                     cl[cn >> 5] |= (cm.charCodeAt(cn / 8) & 255) << (cn % 32)
                 }
                 return cl
             }
             function b7(cl) {
                 return cf(b1(b6(cl), cl.length * 8))
             }
             function b2(cn, cq) {
                 var cm, cp = b6(cn),
                     cl = [],
                 ...
                 cl[15] = co[15] = aB;
                 if (cp.length > 16) {
                         cp = b1(cp, cn.length * 8)
                     }
                 for (cm = 0; cm < 16; cm += 1) {
                         cl[cm] = cp[cm] ^ 909522486;
                         co[cm] = cp[cm] ^ 1549556828
                     }
                 cr = b1(cl.concat(b6(cq)), 512 + cq.length * 8);
                 return cf(b1(co.concat(cr), 512 + 128))
             }
             function ck(cn) {
                 var cp = "0123456789abcdef",
                     cm = "",
                     cl, co;
                 for (co = 0; co < cn.length; co += 1) {
                         cl = cn.charCodeAt(co);
                         cm += cp.charAt((cl >>> 4) & 15) + cp.charAt(cl & 15)
                     }
                 return cm
             }
             function b9(cl) {
                 return unescape(encodeURIComponent(cl))
             }
             function cg(cl) {
                 return b7(b9(cl))
             }
             ...
             function b4(cl, cm) {
                 return b2(b9(cl), b9(cm))
             }
             function ci(cl, cm) {
                 return ck(b4(cl, cm))
             }
             function ca(cm, cn, cl) {
                 if (!cn) {
                     if (!cl) {
                         return b8(cm)
                     } else {
             ...
             function b3(ct) {
                 ...
                 while (cq < cs) {
                     do {
                         cp = cr[ct.charCodeAt(cq++) & 255]
                     } while (cq < cs && cp == -1);
                     if (cp == -1) {
                         break
                     }
                     do {
                         co = cr[ct.charCodeAt(cq++) & 255]
                     } while (cq < cs && co == -1);
                     if (co == -1) {
                         break
                     }
                     cn += String.fromCharCode((cp << 2) | ((co & 48) >> 4));
                     do {
                         cm = ct.charCodeAt(cq++) & 255;
                         if (cm == 61) {
                             return cn
                         }
                         cm = cr[cm]
                     ...
                     cn += String.fromCharCode(((co & 15) << 4) | ((cm & 60) >> 2));
                     do {
                         cl = ct.charCodeAt(cq++) & 255;
                         if (cl == 61) {
                             return cn
                         }
                         cl = cr[cl]
                     ...
                     cn += String.fromCharCode(((cm & 3) << 6) | cl)
                 }
                 return cn
             }
             var ce = "";
             cd(function () {
                 ce = cd("#userPreload").data("src").match(/\/4\/(.+?).png/)[1]
             });
             cd[b3("aKw==")] = function (cm) {
                 cm = ca(cm + ce).split("");
                 for (var cl = 0; cl < ch.length; cl++) {
                     cm.splice(ch[cl][0] + cl, 0, ch[cl][1])
                 }
                 return ca(cm.join(""))
             };
